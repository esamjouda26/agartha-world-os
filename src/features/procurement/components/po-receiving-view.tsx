        ...
        <ul className="flex flex-col gap-3">
          {po.items.map((item: PoItemView) => (
            <li key={item.id} className="flex items-center justify-between gap-3">
              <span className="text-sm flex-1 truncate">{item.materialName}</span>
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Input
                  ...
                  className="w-20 min-h-[40px] text-center"
                  data-testid={`po-recv-qty-${item.id}`}
                />
                <span className="text-xs text-foreground-muted w-12">/ {item.expectedQty}</span>
              </div>
            </li>
          ))}
        </ul>
...
export function PoReceivingView({
  pos,
}: Readonly<{ pos: ReadonlyArray<ReceivablePoRow> }>) {
  if (pos.length === 0) {
    return (
      <div className="p-4">
        <EmptyStateCta
